<script>
    ...
    async function fetchProduct() {
        loading = true;
        try {
            const data = await myFetch(`${PUBLIC_API_URL}/product/product-listings/?product_id=${id}`);
    ...
    async function fetchGalleryImages(listingId) {
        try {
            const data = await myFetch(`${PUBLIC_API_URL}/product/product-listing-images/?product_listing_id=${listingId}`);
    ...
    function handleEditProduct() {
        if (!product?.product_id && !product?.product?.id) return;
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                            ...
                            <div class="px-6 py-6">
                                ...
                                {:else}
                                    ...
                                    <p class="mt-1 text-sm text-gray-500">No product images available.</p>
                                    </div>
                                {/if}
                            </div>
                        </div>
