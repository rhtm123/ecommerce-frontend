  ...
  <footer class="bg-primary py-12 text-gray-200 relative overflow-hidden">
    ...
    {#if mounted}
      <div class="absolute top-4 right-4" in:fly="{{ y: -20, duration: 1000, delay: 200 }}">
        <svg class="w-6 h-6 text-yellow-300" viewBox="0 0 24 24">
          <path fill="currentColor" d="M12,17.27L18.18,21L16.54,13.97L22,9.24L14.81,8.62L12,2L9.19,8.62L2,9.24L7.45,13.97L5.82,21L12,17.27Z" />
        </svg>
      </div>
      <div class="absolute top-8 right-12" in:fly="{{ y: -20, duration: 1000, delay: 400 }}">
        <svg class="w-4 h-4 text-yellow-300" viewBox="0 0 24 24">
          <path fill="currentColor" d="M12,17.27L18.18,21L16.54,13.97L22,9.24L14.81,8.62L12,2L9.19,8.62L2,9.24L7.45,13.97L5.82,21L12,17.27Z" />
        </svg>
      </div>
      <div class="absolute top-2 right-20" in:fly="{{ y: -20, duration: 1000, delay: 600 }}">
        <svg class="w-5 h-5 text-yellow-300" viewBox="0 0 24 24">
          <path fill="currentColor" d="M12,17.27L18.18,21L16.54,13.97L22,9.24L14.81,8.62L12,2L9.19,8.62L2,9.24L7.45,13.97L5.82,21L12,17.27Z" />
        </svg>
      </div>
    {/if}
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div class="space-y-4">
          ...
          <div class="flex space-x-3">
            {#if mounted}
              {#each ['mastercard', 'paypal', 'amex', 'visa'] as card, i}
                <img 
        ...
        <!-- My Account -->
        <div>
          <h3 class="text-white font-semibold mb-4">MY ACCOUNT</h3>
          <ul class="space-y-2">
            {#each ['About', 'My Account', 'Contact', 'Reviews', 'Return Policy', 'Shipping'] as item}
              <li>
                <a 
  ...
  <ul class="space-y-2">
    ...
    {/each}
        ...
        <div>
          ...
          <p class="mb-4">Sign up for newsletter and get 10% cash back offer</p>
          <form on:submit|preventDefault={handleSubmit} class="relative">
            <input
              type="email"
              bind:value={email}
              placeholder="Enter your email"
              class="w-full px-4 py-2 rounded-full bg-white text-gray-900 pr-12"
            />
            <button 
              type="submit"
              class="absolute right-1 top-1 bg-red-500 hover:bg-red-600 text-white p-2 rounded-full transition-colors"
            >
      ...
      <div class="mt-12 pt-8 border-t border-gray-700 text-center">
        <p>Copyright © 2024 KB - All Rights Reserved.</p>
      </div>
    </div>
    ...
    <div class="absolute bottom-0 left-0">
      <svg width="120" height="120" viewBox="0 0 120 120" fill="none">
        <path d="M0 120C0 53.7258 53.7258 0 120 0" stroke="currentColor" stroke-width="2" />
      </svg>
    </div>
  </footer>
